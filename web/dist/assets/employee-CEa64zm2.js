import"./api-BegXHxB0.js";let d=null,z={personnel:[],clients:[],services:[]},w={tab:"myjobs",period:"month",ticketsFilter:"all"};async function J(){if(initTheme(),d=requireAuth("employee"),!d)return;const[t,s,i]=await Promise.all([apiGet("/personnel"),apiGet("/clients"),apiGet("/services")]);z.personnel=t,z.clients=s,z.services=i,R()}function N(t){const s=z.clients.find(i=>i._id===t);return s?s.name:"—"}function B(){return d&&(/mansi/i.test(d.name)||/urna/i.test(d.name))}const M=[{key:"myjobs",label:"My Jobs",icon:"📋"},{key:"dailytasks",label:"Daily Tasks",icon:"✅"},{key:"tickets",label:"Support Tickets",icon:"🎫"},{key:"targets",label:"My Targets",icon:"🎯"}];function R(){const t=document.getElementById("app"),s=M.find(i=>i.key===w.tab)||M[0];t.innerHTML=renderAppShell({user:d,currentRole:"employee",activeTab:w.tab,tabs:M,title:s.label,subtitle:B()?"Lead Workspace · Mansi & Urna Management":"Employee Workspace & Daily Task Checklist"}),bindAppShellEvents(i=>{w.tab=i,R()}),g()}window.ci360NavTab=t=>{w.tab=t,R()};async function g(){const t=document.getElementById("content");if(t){t.innerHTML=renderSkeletonCards(3);try{w.tab==="myjobs"?await W(t):w.tab==="dailytasks"||w.tab==="mytasks"?await L(t):w.tab==="tickets"?await P(t):w.tab==="targets"&&await V(t)}catch(s){t.innerHTML=renderEmptyState("Something went wrong",s.message,"⚠️")}}}function F(){return renderPeriodPicker(w.period)}function j(){document.querySelectorAll("[data-period]").forEach(t=>{t.onclick=()=>{w.period=t.dataset.period,g()}})}async function W(t){const s=await apiGet("/jobs?mine=true"),i={Medium:"gray",High:"amber",Urgent:"red"};if(s.length===0){t.innerHTML=`
      <div class="block">
    ...
    <div class="block">
      ...
      <div style="display:flex;flex-direction:column;gap:14px">
        ${s.map(e=>{var m,k,x;const o=(e.assignments||[]).find(h=>String(h.personId)===String(d.personnelId._id||d.personnelId)),c=String(e.createdBy)===String(d._id),l=e.status==="Completed",b=e.clientApproval&&e.clientApproval.status==="Approved",y=e.clientApproval&&e.clientApproval.status==="Revision Requested",u=e.completionDate?new Date(e.completionDate).toISOString().slice(0,10):"",p=i[e.priority||"Medium"]||"gray";return`
          <div class="job-card" style="background:var(--bg-card);border:1px solid var(--border-sm);border-radius:var(--r-md);padding:20px 22px;box-shadow:var(--shadow-xs);transition:all var(--t-fast);border-left:4px solid ${b?"var(--green-500)":y?"var(--red-500)":l?"var(--green-500)":"var(--brand-500)"}">
            <!-- Header row -->
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:12px;flex-wrap:wrap">
              <div style="min-width:0">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:6px;align-items:center">
                  <span class="badge ${l?"green":"amber"}">${l?"✓ Completed":"⏳ In Progress"}</span>
              ...
              <div style="display:flex;gap:8px;flex-shrink:0;align-items:center">
                <!-- Toggle Status -->
                <button type="button" class="btn ${l?"secondary":"primary"} small"
                        onclick="toggleStatus('${e._id}', ${!l})"
            ...
            ${y?`
              <div style="background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.3);border-radius:var(--r-sm);padding:12px 14px;margin-bottom:14px">
                ...
                ${(x=(k=(e.clientApproval.revisions||[]).slice(-1)[0])==null?void 0:k.attachments)!=null&&x.length?`
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:12px;margin-bottom:14px">
              <div>
                ...
                <div style="font-size:13px;font-weight:600;color:var(--text-1)">${escapeHtml(N(e.clientId))}</div>
              </div>
              <div>
              ...
              <div class="emp-del-form" id="empDelForm-${e._id}" style="display:none;margin-top:10px">
                ...
                <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:8px;margin-top:10px">
                  <label style="display:inline-flex;align-items:center;gap:6px;font-size:12px;color:var(--text-2);cursor:pointer;font-weight:600">
                    <input type="checkbox" id="empDelMarkComplete-${e._id}" ${l?"":"checked"} style="cursor:pointer;width:14px;height:14px;accent-color:var(--brand-600)">
    ...
    </div>`,document.querySelectorAll(".emp-completion-date").forEach(e=>{e.addEventListener("focus",()=>{e.style.borderColor="var(--border-focus)",e.style.boxShadow="0 0 0 3px var(--accent-ring)"}),e.addEventListener("blur",()=>{e.style.borderColor="",e.style.boxShadow=""})}),s.forEach(e=>{bindAttachmentUploader("empDelUp-"+e._id)}),document.querySelectorAll(".emp-del-toggle").forEach(e=>{e.onclick=()=>{const o=document.getElementById("empDelForm-"+e.dataset.id);if(o){const c=o.style.display==="none";o.style.display=c?"block":"none",e.textContent=c?"✕ Close":"+ Add Deliverables"}}}),document.querySelectorAll(".emp-del-cancel").forEach(e=>{e.onclick=()=>{const o=document.getElementById("empDelForm-"+e.dataset.id);o&&(o.style.display="none");const c=document.querySelector(`.emp-del-toggle[data-id="${e.dataset.id}"]`);c&&(c.textContent="+ Add Deliverables")}}),document.querySelectorAll(".emp-del-submit").forEach(e=>{e.onclick=async()=>{var y,u;const o=e.dataset.id,c=getUploaderAttachments("empDelUp-"+o),l=((y=(document.getElementById("empDelNotes-"+o)||{}).value)==null?void 0:y.trim())||"",b=((u=document.getElementById("empDelMarkComplete-"+o))==null?void 0:u.checked)||!1;if(!c.length){flashToast("Please upload or attach at least one file",!0);return}e.disabled=!0,e.textContent="Submitting…";try{const p=c.map(m=>({...m,notes:l}));await apiPost(`/jobs/${o}/deliverables`,{deliverables:p,markComplete:b}),flashToast(b?"Deliverables submitted & job marked as Completed! 📦🎉":"Deliverables submitted successfully! 📦"),g()}catch(p){flashToast(p.message,!0)}finally{e.disabled=!1,e.textContent="Submit Deliverables"}}}),s.forEach(e=>bindSupportTicketSection(e._id,B()))}let v={date:new Date().toISOString().slice(0,10),filter:"all",search:""};async function U(){try{const t=await apiGet("/tasks");if(Array.isArray(t))return localStorage.setItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"),JSON.stringify(t)),t}catch(t){console.warn("Backend /api/tasks offline, using local fallback:",t)}try{const t=localStorage.getItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"));return t?JSON.parse(t):[]}catch{return[]}}async function O(t,s=null){let i=null;try{s?i=await apiPut("/tasks/"+s,t):i=await apiPost("/tasks",t)}catch(o){console.warn("API task save failed, writing local backup:",o)}let e=[];try{const o=localStorage.getItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"));e=o?JSON.parse(o):[]}catch{}if(s){const o=e.findIndex(c=>c._id===s);o>=0&&(e[o]=i||{...e[o],...t,updatedAt:new Date})}else i||(i={_id:"loc_"+Date.now(),...t,createdAt:new Date,completedAt:t.status==="Completed"?new Date:null}),e.unshift(i);return localStorage.setItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"),JSON.stringify(e)),i}async function G(t){try{await apiDelete("/tasks/"+t)}catch(s){console.warn("API task delete failed, updating storage:",s)}try{const s=localStorage.getItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"));let i=s?JSON.parse(s):[];i=i.filter(e=>e._id!==t),localStorage.setItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"),JSON.stringify(i))}catch{}}async function Q(t){try{return await apiPatch("/tasks/"+t+"/toggle",{})}catch(s){console.warn("API toggle failed, toggling locally:",s)}try{const s=localStorage.getItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"));let i=s?JSON.parse(s):[];const e=i.find(o=>o._id===t);if(e){const o=e.status!=="Completed";return e.status=o?"Completed":"Todo",e.completedAt=o?new Date:null,localStorage.setItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"),JSON.stringify(i)),e}}catch{}return null}function H(t,s){let i;if(!t||t==="all")i=new Date;else{const l=t.split("-");l.length===3?i=new Date(Number(l[0]),Number(l[1])-1,Number(l[2])):i=new Date}i.setDate(i.getDate()+s);const e=i.getFullYear(),o=String(i.getMonth()+1).padStart(2,"0"),c=String(i.getDate()).padStart(2,"0");return`${e}-${o}-${c}`}function Y(t){if(!t||t==="all")return"All Days Checklist";const s=t.split("-"),i=s.length===3?new Date(Number(s[0]),Number(s[1])-1,Number(s[2])):new Date,e=new Date;e.setHours(0,0,0,0);const o=new Date(i);o.setHours(0,0,0,0);const c=Math.round((o-e)/(1e3*60*60*24)),l=i.toLocaleDateString("en-US",{weekday:"long",month:"short",day:"numeric",year:"numeric"});return c===0?`Today · ${l}`:c===-1?`Yesterday · ${l}`:c===1?`Tomorrow · ${l}`:l}async function L(t){const s=await U(),i=new Date,e=`${i.getFullYear()}-${String(i.getMonth()+1).padStart(2,"0")}-${String(i.getDate()).padStart(2,"0")}`,o=v.date;let c=s;o!=="all"&&(c=s.filter(r=>r.dueDate?new Date(r.dueDate).toISOString().slice(0,10)===o:o===e));const l=c.length,b=c.filter(r=>r.status==="Completed").length,y=l-b,u=l>0?Math.round(b/l*100):0,p=l>0&&b===l;let m=c;if(v.filter==="active"?m=m.filter(r=>r.status!=="Completed"):v.filter==="completed"&&(m=m.filter(r=>r.status==="Completed")),v.search){const r=v.search.toLowerCase();m=m.filter($=>($.title||"").toLowerCase().includes(r))}const k=o===e;t.innerHTML=`
    <div class="block" style="max-width:880px;margin:0 auto">
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;flex-wrap:wrap;gap:12px">
        <div>
          <h2 style="margin:0 0 4px;display:flex;align-items:center;gap:8px">
            ...
            <span class="eyebrow">${l} ${l===1?"task":"tasks"}</span>
          </h2>
        ...
        <div class="daily-date-nav-bar">
          ...
          <button type="button" class="btn ${k?"primary":"ghost"} small" id="dailyTodayBtn" style="padding:4px 10px;font-size:12px;font-weight:700">
            📅 Today
          </button>
      ...
      <div class="daily-checklist-card">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;flex-wrap:wrap;gap:8px">
            ...
            <div style="display:flex;align-items:center;gap:8px">
              <span style="font-size:12.5px;font-weight:700;color:${p?"var(--green-600)":"var(--text-2)"}">
                ${l>0?`${b} of ${l} completed (${u}%)`:"No tasks yet"}
              </span>
            </div>
          </div>
        ...
        <div style="padding:20px 24px 10px">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:10px">
            <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
              <button class="pchip ${v.filter==="all"?"active":""}" data-df="all">All (${l})</button>
              <button class="pchip ${v.filter==="active"?"active":""}" data-df="active">To Do (${y})</button>
              <button class="pchip ${v.filter==="completed"?"active":""}" data-df="completed">✓ Completed (${b})</button>
            </div>
            ...
            <div style="display:flex;gap:8px;align-items:center">
              <div class="ticket-search-box" style="margin:0">
                ...
                <input type="text" id="dailySearchInp" placeholder="Search checklist…" value="${escapeHtml(v.search)}" style="font-size:12px;padding:4px 6px">
                ${v.search?'<button type="button" id="dailyClearSearch" style="background:none;border:none;color:var(--text-4);cursor:pointer;font-size:11px">✕</button>':""}
              </div>
        ...
        <div id="dailyChecklistRowsContainer" style="border-top:1px solid var(--border-xs)">
          ...
          `:m.map(r=>{const $=r.status==="Completed",_=r.dueDate?new Date(r.dueDate).toISOString().slice(0,10):"";return`
              <div class="daily-item-row ${$?"completed":""}" id="daily-row-${r._id}">
                <!-- Circle checkbox -->
                <div style="display:flex;align-items:center;gap:14px;flex:1;min-width:0">
                  <button type="button" class="daily-circle-check ${$?"checked":""}" onclick="toggleDailyTask('${r._id}')" title="${$?"Mark Incomplete":"Mark Complete"}">
                    ✓
                  </button>
                  <span class="daily-item-text" onclick="toggleDailyTask('${r._id}')" style="cursor:pointer;flex:1">
                    ${escapeHtml(r.title)}
                  </span>
                </div>
                ...
                <div class="daily-actions-hover">
                  ...
                  `:""}
                  <button type="button" class="btn ghost small" onclick="editDailyTask('${r._id}')" title="Edit task" style="padding:3px 7px;font-size:12px">✏️</button>
                  <button type="button" class="btn ghost small" onclick="deleteDailyTask('${r._id}')" title="Delete task" style="padding:3px 7px;font-size:12px;color:var(--red-500)">🗑️</button>
                </div>
              </div>
            `}).join("")}
        </div>
  ...
  `;const x=document.getElementById("dailyTaskQuickInput"),h=document.getElementById("dailyTaskAddBtn"),T=async()=>{const r=((x==null?void 0:x.value)||"").trim();if(!r){flashToast("Please enter a task",!0);return}const $=v.date==="all"?e:v.date;try{await O({title:r,status:"Todo",dueDate:$}),flashToast("Task added! ✍️"),x&&(x.value="",x.focus()),g()}catch(_){flashToast(_.message,!0)}};h&&(h.onclick=T),x&&(x.onkeydown=r=>{r.key==="Enter"&&T()}),document.querySelectorAll("[data-df]").forEach(r=>{r.onclick=()=>{v.filter=r.dataset.df,g()}});const C=document.getElementById("dailyPrevDayBtn");C&&(C.onclick=()=>{v.date=H(v.date,-1),g()});const A=document.getElementById("dailyNextDayBtn");A&&(A.onclick=()=>{v.date=H(v.date,1),g()});const a=document.getElementById("dailyTodayBtn");a&&(a.onclick=()=>{v.date=e,g()});const n=document.getElementById("dailyAllDatesBtn");n&&(n.onclick=()=>{v.date=v.date==="all"?e:"all",g()});const f=document.getElementById("dailyDatePickerInp");f&&(f.onchange=r=>{r.target.value&&(v.date=r.target.value,g())});const S=document.getElementById("dailySearchInp");S&&(S.oninput=r=>{v.search=r.target.value,L(t)});const D=document.getElementById("dailyClearSearch");D&&(D.onclick=()=>{v.search="",g()})}window.toggleDailyTask=async function(t){try{await Q(t),g()}catch(s){flashToast(s.message,!0)}};window.deleteDailyTask=async function(t){try{await G(t),flashToast("Task deleted"),g()}catch(s){flashToast(s.message,!0)}};window.editDailyTask=async function(t){const i=(await U()).find(o=>o._id===t);if(!i)return;const e=prompt("Edit task:",i.title);if(e!==null&&e.trim())try{await O({title:e.trim()},t),flashToast("Task updated"),g()}catch(o){flashToast(o.message,!0)}};window.clearCompletedDailyTasks=async function(){if(confirm("Delete all completed tasks from this view?"))try{await apiPost("/tasks/clear-completed",{date:v.date});try{const t=localStorage.getItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"));if(t){let s=JSON.parse(t);s=s.filter(i=>i.status!=="Completed"),localStorage.setItem("ci360_tasks_"+((d==null?void 0:d._id)||"user"),JSON.stringify(s))}}catch{}flashToast("Completed tasks deleted! 🗑️"),g()}catch(t){flashToast(t.message,!0)}};let I="",E="all";async function P(t){let s=await apiGet("/tickets");s.sort((a,n)=>new Date(n.createdAt)-new Date(a.createdAt));const i=B(),e=s.length,o=s.filter(a=>a.status==="Open").length,c=s.filter(a=>a.status==="In Review").length,l=s.filter(a=>a.status==="Resolved"||a.status==="Closed").length,b=e>0?Math.round(l/e*100):100,y=w.ticketsFilter||"all";let u=s;if(y==="open"?u=u.filter(a=>a.status==="Open"):y==="in-review"?u=u.filter(a=>a.status==="In Review"):y==="resolved"?u=u.filter(a=>a.status==="Resolved"):y==="closed"&&(u=u.filter(a=>a.status==="Closed")),E!=="all"&&(u=u.filter(a=>a.priority===E)),I){const a=I.toLowerCase();u=u.filter(n=>{const f=n.jobId&&n.jobId.title||"";return(n.subject||"").toLowerCase().includes(a)||(n.message||"").toLowerCase().includes(a)||(n.userName||"").toLowerCase().includes(a)||f.toLowerCase().includes(a)})}const p={Open:"red","In Review":"amber",Resolved:"green",Closed:"gray"},m={Low:"green",Medium:"gray",High:"amber",Urgent:"red"};function k(a){if(!a)return"U";const n=a.trim().split(/\s+/);return n.length===1?n[0].slice(0,2).toUpperCase():(n[0][0]+n[n.length-1][0]).toUpperCase()}function x(a){if(!a)return"";const n=new Date,f=new Date(a),S=Math.floor((n-f)/1e3);if(S<60)return"Just now";const D=Math.floor(S/60);if(D<60)return`${D}m ago`;const r=Math.floor(D/60);if(r<24)return`${r}h ago`;const $=Math.floor(r/24);return $<7?`${$}d ago`:fmtDate(a)}t.innerHTML=`
    <section class="block">
      <!-- Top KPIs -->
      ...
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;flex-wrap:wrap;gap:10px">
        <div style="display:flex;gap:6px;flex-wrap:wrap;align-items:center">
          ...
          <button class="pchip ${y==="resolved"?"active":""}" data-tf="resolved">🟢 Resolved (${l})</button>
      ...
      <div style="display:flex;flex-direction:column;gap:14px">
        ${u.map(a=>{const n=a.jobId?a.jobId.title||"Untitled Job":"Assigned Job",f=(a.status||"Open").toLowerCase().replace(" ","-"),S=a.status==="Open",D=(a._id||"").slice(-4).toUpperCase(),r=k(a.userName);return`
          <div class="ticket-card status-${f}" id="emp-tk-${a._id}">
            <div class="ticket-card-header">
              ...
              <div class="ticket-meta-badges">
                <span class="badge ${p[a.status]||"gray"}">
                  ${S?'<span class="pulse-dot"></span>':""} ${escapeHtml(a.status)}
                </span>
                <span class="badge ${m[a.priority]||"gray"}">${escapeHtml(a.priority)}</span>
              </div>
            </div>

            <div class="ticket-author-row">
              <div class="ticket-avatar">${r}</div>
              <div class="ticket-author-meta">
                ...
                <span class="ticket-time-ago">${x(a.createdAt)} · ${fmtDate(a.createdAt)}</span>
              </div>
            </div>
              ...
              <div class="ticket-thread-wrap">
                <div class="ticket-admin-reply-card">
                  <div class="ticket-admin-reply-header">
                    ...
                    ${a.repliedAt?`<span style="font-size:11px;color:var(--text-4)">${x(a.repliedAt)}</span>`:""}
                  </div>
            ...
            <div class="ticket-toolbar">
              ...
              <select class="emp-tk-status-sel" data-tkid="${a._id}" style="font-size:12px;padding:5px 8px;border:1px solid var(--border-sm);border-radius:var(--r-sm);background:var(--bg-surface);color:var(--text-1)">
              ...
              <button class="btn ghost small emp-tk-reply-toggle" data-tkid="${a._id}" type="button">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                ${a.adminReply?"Edit Reply":"💬 Reply"}
              </button>

              ${a.status!=="Resolved"?`
                <button class="btn ghost small emp-tk-quick-resolve" data-tkid="${a._id}" type="button" style="color:var(--green-600);border-color:var(--green-400)">
                  ✓ Quick Resolve
                </button>`:""}

              <div class="ticket-reply-form" id="emp-tk-replyform-${a._id}">
                <div class="ticket-templates-bar">
                  ...
                  <button type="button" class="ticket-template-btn" data-tkid="${a._id}" data-tpl="Mansi & Urna team is on it! Updates will be shared shortly.">🚀 On It</button>
                ...
                <textarea id="emp-tk-replytxt-${a._id}" rows="2" placeholder="Write response to ticket..." style="font-size:13px;padding:8px 10px;border:1px solid var(--border-sm);border-radius:var(--r-sm);background:var(--bg-surface);color:var(--text-1);resize:vertical;width:100%;box-sizing:border-box">${escapeHtml(a.adminReply||"")}</textarea>
                <div style="display:flex;justify-content:flex-end;gap:6px;margin-top:6px">
                  <button class="btn ghost small emp-tk-reply-cancel" data-tkid="${a._id}" type="button">Cancel</button>
                  <button class="btn gold small emp-tk-reply-save" data-tkid="${a._id}" type="button">Save Response</button>
                </div>
              </div>
            </div>`:""}
          </div>`}).join("")}
  ...
  `,document.querySelectorAll("[data-tf]").forEach(a=>{a.onclick=()=>{w.ticketsFilter=a.dataset.tf,g()}});const h=document.getElementById("empTkSearch");h&&(h.oninput=a=>{I=a.target.value,P(t)});const T=document.getElementById("empClearSearch");T&&(T.onclick=()=>{I="",P(t)});const C=document.getElementById("empTkPriFilter");C&&(C.onchange=a=>{E=a.target.value,P(t)});const A=document.getElementById("empRaiseTicketGlobalBtn");A&&(A.onclick=async()=>{const a=await apiGet("/jobs?mine=true");if(!a.length){flashToast("No jobs assigned to raise tickets against.",!0);return}const n=openModal(`
        ...
        <div class="field" style="margin-bottom:12px">
          ...
          <select id="empModalTkJob" style="width:100%;font-size:13.5px;padding:10px 12px;border:1px solid var(--border-sm);border-radius:var(--r-md);background:var(--bg-surface);color:var(--text-1)">
            ...
            ${a.map(f=>`<option value="${f._id}">${escapeHtml(f.title||"Untitled Job")} (${escapeHtml(N(f.clientId))})</option>`).join("")}
          </select>
        </div>
      ...
      `);n.querySelector("#mEmpCancelTicket").onclick=()=>n.remove(),n.querySelector("#mEmpSubmitTicket").onclick=async()=>{const f=n.querySelector("#empModalTkJob").value,S=n.querySelector("#empModalTkSub").value.trim(),D=n.querySelector("#empModalTkPri").value,r=n.querySelector("#empModalTkMsg").value.trim();if(!f){flashToast("Please select a job",!0);return}if(!S){flashToast("Please enter an issue subject",!0);return}if(!r){flashToast("Please enter description",!0);return}try{await apiPost("/tickets",{jobId:f,subject:S,message:r,priority:D}),flashToast("Support Ticket Raised! 🎫"),n.remove(),P(t)}catch($){flashToast($.message,!0)}}}),i&&(document.querySelectorAll(".emp-tk-quick-resolve").forEach(a=>{a.onclick=async()=>{try{await apiPut("/tickets/"+a.dataset.tkid,{status:"Resolved"}),flashToast("Ticket marked as Resolved! 🎉"),g()}catch(n){flashToast(n.message,!0)}}}),document.querySelectorAll(".ticket-template-btn").forEach(a=>{a.onclick=()=>{const n=document.getElementById("emp-tk-replytxt-"+a.dataset.tkid);n&&(n.value=a.dataset.tpl,n.focus())}}),document.querySelectorAll(".emp-tk-status-sel").forEach(a=>{a.onchange=async()=>{try{await apiPut("/tickets/"+a.dataset.tkid,{status:a.value}),flashToast("Status updated"),g()}catch(n){flashToast(n.message,!0)}}}),document.querySelectorAll(".emp-tk-reply-toggle").forEach(a=>{a.onclick=()=>{const n=document.getElementById("emp-tk-replyform-"+a.dataset.tkid);n&&n.classList.toggle("show")}}),document.querySelectorAll(".emp-tk-reply-cancel").forEach(a=>{a.onclick=()=>{const n=document.getElementById("emp-tk-replyform-"+a.dataset.tkid);n&&n.classList.remove("show")}}),document.querySelectorAll(".emp-tk-reply-save").forEach(a=>{a.onclick=async()=>{const n=document.getElementById("emp-tk-replytxt-"+a.dataset.tkid);if(n)try{await apiPut("/tickets/"+a.dataset.tkid,{adminReply:n.value.trim()}),flashToast("Response saved! 🛡️"),g()}catch(f){flashToast(f.message,!0)}}}))}window.saveCompDate=async function(t){try{const s=document.querySelector(`.emp-completion-date[data-id="${t}"]`);if(!s)return;const i=s.value?s.value:null;await apiPut("/jobs/"+t,{completionDate:i,status:i?"Completed":"In Progress"}),flashToast("Delivery date & status saved!"),g()}catch(s){flashToast(s.message,!0)}};window.toggleStatus=async function(t,s){try{const i=document.querySelector(`.emp-completion-date[data-id="${t}"]`);let e=s?(i==null?void 0:i.value)||new Date().toISOString().slice(0,10):null;i&&(i.value=e||""),await apiPut("/jobs/"+t,{status:s?"Completed":"In Progress",completionDate:e}),flashToast(s?"Job marked as Completed! 🎉":"Job reopened as In Progress"),g()}catch(i){flashToast(i.message,!0)}};window.delJob=async function(t){if(confirm("Delete this job entry? This cannot be undone."))try{await apiDelete("/jobs/"+t),flashToast("Job deleted"),g()}catch(s){flashToast(s.message,!0)}};async function V(t){const[s,i]=await Promise.all([apiGet("/dashboard/employee?period="+w.period),apiGet("/targets?mine=true")]),e=z.personnel.find(p=>{var m;return String(p._id)===String(((m=d.personnelId)==null?void 0:m._id)||d.personnelId)}),o=e?e.capacity:48,c=s.target?s.target.targetHours:Math.round(o*.85),l=Math.round(s.hours/(c||1)*100),b=l>=100?"green":l>=75?"amber":"red",y=Math.min(l,100);t.innerHTML=`
    <div class="block">
      ...
      <div class="grid grid-4" style="margin-bottom:20px">
        ...
        <div class="card kpi">
          ...
          <div class="kpi-sub">${l}% completed</div>
        </div>
        <div class="card kpi">
      ...
      <div class="card" style="margin-bottom:24px">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;flex-wrap:wrap;gap:8px">
          <div>
            <div style="font-size:13.5px;font-weight:700;color:var(--text-1)">Overall Hours Target (${w.period})</div>
            <div style="font-size:12px;color:var(--text-3);margin-top:2px">${fmtHours(s.hours)} logged of ${fmtHours(c)} target capacity</div>
          </div>
          <span class="badge ${b}" style="font-size:12.5px;padding:4px 10px">${l}% Achieved</span>
        </div>
      ...
      `:`
        <div class="grid grid-2" style="gap:16px">
          ${i.map(p=>{var f;const m=((f=p.serviceId)==null?void 0:f.name)||"General Service",x={count:"Deliverables",hours:"Hours",reels:"Reels",stories:"Stories",posts:"Posts"}[p.unit]||(p.unit?p.unit.charAt(0).toUpperCase()+p.unit.slice(1):"Deliverables"),h=p.actual||0,T=p.quantity||1,C=T>0?h/T:0,A=Math.round(C*100),a=Math.min(A,100),n=C>=1?{l:"🟢 On Pace",c:"green"}:C>=.6?{l:"🟡 Behind",c:"amber"}:{l:"🔴 Off Pace",c:"red"};return`
            <div class="card" style="display:flex;flex-direction:column;justify-content:space-between;gap:12px;border:1px solid var(--border-sm);position:relative">
              <div>
                ...
                <div class="progress-bar-wrap" style="height:8px;border-radius:var(--r-full);background:var(--bg-surface);overflow:hidden;margin:6px 0 10px">
                  <div class="progress-bar-fill ${n.c}" style="width:${a}%;height:100%;border-radius:var(--r-full);background:var(--${n.c==="green"?"green":"amber"}-500);transition:width 0.8s ease"></div>
                </div>
      ...
      `}
    </div>`;const u=document.getElementById("addEmpTargetBtn");u&&(u.onclick=()=>q(null)),document.querySelectorAll(".edit-emp-target").forEach(p=>{p.onclick=()=>q(i.find(m=>m._id===p.dataset.id))}),j()}window.stepTargetCompleted=function(t,s){const i=document.querySelector(`.emp-target-completed-inp[data-id="${t}"]`);if(!i)return;let e=Math.max(0,(Number(i.value)||0)+s);i.value=e,saveTargetCompleted(t)};window.saveTargetCompleted=async function(t){try{const s=document.querySelector(`.emp-target-completed-inp[data-id="${t}"]`);if(!s)return;const i=Number(s.value)||0;await apiPut("/targets/"+t,{completed:i}),flashToast("Target progress updated! 🎯"),g()}catch(s){flashToast(s.message,!0)}};function q(t){var p,m;const s=!t,i=z.services||[];t=t||{serviceId:(p=i[0])==null?void 0:p._id,quantity:5,unit:"reels",period:"day",completed:0,attachments:[]};const e=((m=t.serviceId)==null?void 0:m._id)||t.serviceId||"",c=["reels","stories","posts","count","hours"].includes(t.unit),l=openModal(`
    ...
    <div class="field" style="margin-bottom:12px">
      ...
      <select id="mEmpTgtService">
        ${i.map(k=>`<option value="${k._id}" ${String(k._id)===String(e)?"selected":""}>${escapeHtml(k.name)}</option>`).join("")}
      </select>
    </div>
    ...
    <div class="log-job-row" style="margin-bottom:12px">
      <div class="field" style="margin-bottom:0">
        ...
        <select id="mEmpTgtUnit">
          <option value="reels" ${c&&t.unit==="reels"?"selected":""}>🎬 Reels</option>
          <option value="stories" ${c&&t.unit==="stories"?"selected":""}>📱 Stories</option>
          <option value="posts" ${c&&t.unit==="posts"?"selected":""}>🖼️ Posts</option>
          <option value="count" ${c&&t.unit==="count"?"selected":""}>📦 Deliverables / Jobs Count</option>
          <option value="hours" ${c&&t.unit==="hours"?"selected":""}>⏱️ Hours Spent</option>
    ...
    <div class="field" id="mEmpTgtCustomUnitWrap" style="margin-bottom:16px;display:${c?"none":"flex"}">
      ...
      <input type="text" id="mEmpTgtCustomUnit" placeholder="e.g. Shorts, Banners, Thumbnails, Articles…" value="${c?"":escapeHtml(t.unit||"")}">
    </div>
  ...
  `);bindAttachmentUploader("mEmpTgtAttachments",{existing:t.attachments||[]});const b=l.querySelector("#mEmpTgtUnit"),y=l.querySelector("#mEmpTgtCustomUnitWrap"),u=l.querySelector("#mEmpTgtCustomUnit");b.onchange=()=>{b.value==="custom"?(y.style.display="flex",u.focus()):y.style.display="none"},l.querySelector("#mEmpTgtCancel").onclick=()=>l.remove(),l.querySelector("#mEmpTgtSave").onclick=async()=>{let k=b.value;k==="custom"&&(k=u.value.trim()||"Deliverables");const x=getUploaderAttachments("mEmpTgtAttachments"),h={serviceId:l.querySelector("#mEmpTgtService").value,quantity:Number(l.querySelector("#mEmpTgtQty").value)||1,completed:Number(l.querySelector("#mEmpTgtCompleted").value)||0,unit:k,period:l.querySelector("#mEmpTgtPeriod").value,attachments:x};if(!h.serviceId){flashToast("Service is required",!0);return}try{s?await apiPost("/targets",h):await apiPut("/targets/"+t._id,h),flashToast("Target saved! 🎯"),l.remove(),g()}catch(T){flashToast(T.message,!0)}}}J();
