import"./api-DfJKfy_f.js";let V=null,h={personnel:[],clients:[],services:[],salaryGrades:[],salaryAssignments:[]},R=null,S={tab:"dashboard",period:"month",jobsFilter:"all",ticketsFilter:"all"},g={title:"",clientId:"",serviceIds:[],date:new Date().toISOString().slice(0,10),completion:"",value:"",desc:"",assignments:[{personId:"",percent:100,hours:""},{personId:"",percent:0,hours:""}]};async function me(){initTheme(),V=requireAuth("superadmin"),V&&(await z(),W())}async function z(){const[t,s,e]=await Promise.all([apiGet("/personnel"),apiGet("/clients"),apiGet("/services")]);h.personnel=t,h.clients=s,h.services=e}function F(){const t=h.personnel.find(i=>/mansi/i.test(i.name)),s=h.personnel.find(i=>/urna/i.test(i.name)),e=[];return t&&e.push({personId:t._id,percent:"",hours:""}),s&&e.push({personId:s._id,percent:"",hours:""}),e.length===0&&e.push({personId:"",percent:"",hours:""}),e}function re(t){const s=h.personnel.find(e=>e._id===t);return s?s.name:"—"}function J(t){const s=h.clients.find(e=>e._id===t);return s?s.name:"—"}const G=[{key:"dashboard",label:"Dashboard",icon:"📊"},{key:"dailytasks",label:"Daily Tasks",icon:"✅"},{key:"logjob",label:"Log a Job",icon:"➕"},{key:"jobs",label:"All Jobs",icon:"📁"},{key:"tickets",label:"Support Tickets",icon:"🎫"},{key:"byclient",label:"By Client",icon:"💼"},{key:"byperson",label:"By Person",icon:"👥"},{key:"accounts",label:"Accounts",icon:"🏢"},{key:"targets",label:"Targets",icon:"🎯"},{key:"salaries",label:"Salaries",icon:"💰"},{key:"users",label:"Users",icon:"👤"},{key:"manage",label:"Manage",icon:"⚙️"}];function W(){const t=document.getElementById("app"),s=G.find(e=>e.key===S.tab)||G[0];t.innerHTML=renderAppShell({user:V,currentRole:"superadmin",activeTab:S.tab,tabs:G,title:s.label,subtitle:"Productivity & Revenue Intelligence"}),bindAppShellEvents(e=>{S.tab=e,W()}),x()}window.ci360NavTab=t=>{S.tab=t,W()};async function x(){const t=document.getElementById("content");if(t){t.innerHTML=renderSkeletonCards(4);try{S.tab==="dashboard"?await ue(t):S.tab==="dailytasks"?await pe(t):S.tab==="logjob"?M(t):S.tab==="jobs"?await fe(t):S.tab==="tickets"?await U(t):S.tab==="byclient"?await xe(t):S.tab==="byperson"?await de(t):S.tab==="accounts"?await ke(t):S.tab==="targets"?await $e(t):S.tab==="salaries"?await Se(t):S.tab==="settings"||S.tab==="manage"?Ie(t):S.tab==="users"&&await ce(t)}catch(s){t.innerHTML=`<div class="empty"><h3>Something went wrong</h3>${escapeHtml(s.message)}</div>`}}}function Q(){return renderPeriodPicker(S.period)}function Y(){document.querySelectorAll("[data-period]").forEach(t=>{t.onclick=()=>{S.period=t.dataset.period,x()}})}async function ue(t){R=await apiGet("/dashboard/admin?period="+S.period);const s=R.overview;t.innerHTML=`
  ...
  `,Y()}function X(t,s){if(!t.length)return'<div class="empty">No data yet.</div>';const e=Math.max(1,...t.map(i=>i[1]));return t.map(([i,n])=>`
    ...
    </div>`}function M(t){(!g.assignments||g.assignments.every(e=>!e.personId))&&(g.assignments=F());const s=g.assignments.reduce((e,i)=>e+(Number(i.percent)||0),0);t.innerHTML=`
    <section class="block">
  ...
  `}function be(){const t=document.getElementById("jClient");t&&(t.onchange=v=>g.clientId=v.target.value);const s=document.getElementById("addNewClientBtn");s&&(s.onclick=()=>{const v=openModal(`
      ...
      `);v.querySelector("#mCancel").onclick=()=>v.remove(),v.querySelector("#mSave").onclick=async()=>{const c=v.querySelector("#newServiceName").value.trim();if(!c){flashToast("Service name is required",!0);return}try{const k=await apiPost("/services",{name:c});await z(),g.serviceIds.includes(k._id)||g.serviceIds.push(k._id),flashToast("Service added"),v.remove(),M(document.getElementById("content"))}catch(k){flashToast(k.message,!0)}}});const d=document.getElementById("jDesc");d&&(d.oninput=v=>g.desc=v.target.value);const u=document.getElementById("jValue");u&&(u.oninput=v=>g.value=v.target.value),document.querySelectorAll(".person-assign-row").forEach(v=>{const c=Number(v.dataset.idx),k=v.querySelector(".a-person");k&&(k.onchange=a=>g.assignments[c].personId=a.target.value);const f=v.querySelector(".a-percent");f&&(f.oninput=a=>{g.assignments[c].percent=a.target.value,ye()});const p=v.querySelector(".a-hours");p&&(p.oninput=a=>g.assignments[c].hours=a.target.value);const y=v.querySelector(".a-remove");y&&(y.onclick=()=>{g.assignments.length>1&&(g.assignments.splice(c,1),M(document.getElementById("content")))})});const b=document.getElementById("addAssignRow");b&&(b.onclick=()=>{g.assignments.push({personId:"",percent:0,hours:""}),M(document.getElementById("content"))}),bindAttachmentUploader("jAttachments",{existing:g.attachments||[]});const w=document.getElementById("clearJobBtn");w&&(w.onclick=()=>{g={title:"",assignments:F(),serviceIds:[],clientId:"",date:new Date().toISOString().slice(0,10),completion:"",value:"",desc:"",attachments:[]},setUploaderAttachments("jAttachments",[]),M(document.getElementById("content"))});const I=document.getElementById("saveJobBtn");I&&(I.onclick=async()=>{try{if(!g.clientId){flashToast("Please select a client",!0);return}if(!g.serviceIds||!g.serviceIds.length){flashToast("Please select at least one service",!0);return}const v=g.assignments.filter(a=>a.personId);if(!v.length){flashToast("Please assign at least one person",!0);return}for(const a of v)if(a.hours===""||a.hours==null){flashToast("Enter hours spent for every assigned person",!0);return}const c=h.clients.find(a=>a._id===g.clientId),f=h.services.filter(a=>g.serviceIds.includes(a._id)).map(a=>a.name).join(", "),p=g.title||(c?`${c.name} — ${f||"Deliverable"}`:f||"Untitled Job"),y=getUploaderAttachments("jAttachments");await apiPost("/jobs",{title:p,clientId:g.clientId,serviceIds:g.serviceIds,date:g.date||new Date().toISOString().slice(0,10),completionDate:g.completion||null,value:Number(g.value)||0,description:g.desc||"",assignments:v,attachments:y}),flashToast("Job saved successfully! 📁"),g={title:"",assignments:F(),serviceIds:[],clientId:"",date:new Date().toISOString().slice(0,10),completion:"",value:"",desc:"",attachments:[]},S.tab="jobs",x()}catch(v){flashToast(v.message,!0)}})}function ye(){const t=g.assignments.reduce((e,i)=>e+(Number(i.percent)||0),0),s=document.querySelector(".assign-total");s&&(s.textContent=`${t}% of work allocated ${t!==100?"— should total 100%":"✓"}`,s.classList.toggle("warn",t!==100))}async function fe(t){let s=await apiGet("/jobs");s.sort((e,i)=>new Date(i.date)-new Date(e.date)),S.jobsFilter==="progress"&&(s=s.filter(e=>!e.completionDate)),S.jobsFilter==="done"&&(s=s.filter(e=>e.completionDate)),t.innerHTML=`
    <section class="block">
  ...
  `,document.querySelectorAll("[data-jf]").forEach(e=>e.onclick=()=>{S.jobsFilter=e.dataset.jf,x()}),document.querySelectorAll(".view-job-files").forEach(e=>{e.onclick=()=>{const i=e.dataset.id,n=s.find(b=>b._id===i);if(!n)return;const l=encodeURIComponent(JSON.stringify(n.attachments||[])),d=encodeURIComponent(JSON.stringify(n.deliverables||[])),u=openModal(`
      ...
      `);bindAttachmentUploader("mDelUpload"),u.querySelector("#mDelSaveBtn").onclick=async()=>{const b=getUploaderAttachments("mDelUpload"),w=u.querySelector("#mDelNote").value.trim();if(!b.length){flashToast("Please select at least one deliverable file",!0);return}try{const I=b.map(v=>({...v,notes:w}));await apiPost(`/jobs/${i}/deliverables`,{deliverables:I}),flashToast("Deliverables added! 📦"),u.remove(),x()}catch(I){flashToast(I.message,!0)}},u.querySelector("#mCloseFiles").onclick=()=>u.remove()}}),document.querySelectorAll(".view-job-tickets").forEach(e=>{e.onclick=()=>{const i=e.dataset.id,n=e.dataset.title,l=openModal(`
          ...
          <div class="ticket-card status-${A}" id="adm-tk-${a._id}">
            <div class="ticket-card-header">
              ...
              <div class="ticket-meta-badges">
                ...
                <span class="badge ${I[a.priority]||"gray"}">${escapeHtml(a.priority)}</span>
              </div>
            </div>
      ...
      <div class="card table-card" style="padding:0;overflow:hidden">
        <div class="table-wrapper">
          <table style="width:100%;border-collapse:collapse;table-layout:fixed;min-width:780px">
            ...
            <tbody>
              ${s.map(i=>{var f,p;const n=((f=i.personId)==null?void 0:f.name)||"—",l=((p=i.serviceId)==null?void 0:p.name)||"—",d=we(i.unit),u=e[i.period]||(i.period?i.period.toUpperCase():"Daily"),b=i.actual||0,w=i.quantity>0?b/i.quantity:0,I=Math.round(w*100),v=w>=1?"green":w>=.6?"amber":"red",c=w>=1?"✓ Met":w>=.6?"Behind":"Off Pace",k=Math.min(I,100);return`
                <tr>
  ...
  `);e.querySelector("#mCancel").onclick=()=>e.remove(),e.querySelector("#mSave").onclick=async()=>{const i={name:e.querySelector("#mSName").value.trim(),hours:Number(e.querySelector("#mSHours").value)||0};if(!i.name){flashToast("Name is required",!0);return}try{s?await apiPost("/services",i):await apiPut("/services/"+t._id,i),await z(),e.remove(),x()}catch(n){flashToast(n.message,!0)}}}async function ce(t){const s=await apiGet("/users");let e="all",i="role",n="";function l(){let c=[...s];if(n){const p=n.toLowerCase();c=c.filter(y=>y.name&&y.name.toLowerCase().includes(p)||y.email&&y.email.toLowerCase().includes(p)||y.role&&y.role.toLowerCase().includes(p))}e==="admin"?c=c.filter(p=>p.role==="superadmin"||p.role==="admin"):e==="employee"?c=c.filter(p=>p.role==="employee"):e==="client"&&(c=c.filter(p=>p.role==="client"));const k={superadmin:1,admin:1,employee:2,client:3};i==="role"?c.sort((p,y)=>{const a=k[p.role]||9,m=k[y.role]||9;return a!==m?a-m:(p.name||"").localeCompare(y.name||"")}):i==="role-desc"?c.sort((p,y)=>{const a=k[p.role]||9,m=k[y.role]||9;return a!==m?m-a:(p.name||"").localeCompare(y.name||"")}):i==="name-asc"?c.sort((p,y)=>(p.name||"").localeCompare(y.name||"")):i==="name-desc"?c.sort((p,y)=>(y.name||"").localeCompare(p.name||"")):i==="status"&&c.sort((p,y)=>(y.active?1:0)-(p.active?1:0));const f=t.querySelector("#userTableBody");if(f){if(c.length===0){f.innerHTML='<tr><td colspan="6"><div class="empty" style="padding:32px 16px">No users found matching the selected filters.</div></td></tr>';return}f.innerHTML=c.map(p=>{const y=p.role==="superadmin"||p.role==="admin",a=p.role==="employee",m=y?'<span class="badge gold" style="font-weight:700">👑 Admin</span>':a?'<span class="badge blue" style="font-weight:700">💼 Employee</span>':'<span class="badge green" style="font-weight:700">🤝 Client</span>',A=p.personnelId?escapeHtml(p.personnelId.name):p.clientId?escapeHtml(p.clientId.name):'<span class="muted">—</span>';return`
    ...
    </section>`,l(),t.querySelectorAll(".user-role-filter").forEach(c=>{c.onclick=()=>{t.querySelectorAll(".user-role-filter").forEach(k=>k.classList.remove("active")),c.classList.add("active"),e=c.dataset.role,l()}});const w=t.querySelector("#userSortSel");w&&(w.onchange=()=>{i=w.value,l()});const I=t.querySelector("#userSearchInp");I&&(I.oninput=()=>{n=I.value.trim(),l()});const v=t.querySelector("#addUserBtn");v&&(v.onclick=()=>ne(null))}function ne(t){const s=!t;t=t||{name:"",email:"",role:"employee",personnelId:"",clientId:"",active:!0};const e=t.role,i=openModal(`<h3>${s?"Add":"Edit"} User</h3>
    <div class="field-row">
    ...
    <div class="block">
      ...
      <div class="card" style="padding:14px 18px;margin-bottom:20px;background:var(--bg-card)">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:12px">
          ...
          <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
            <button class="pchip ${T.filter==="all"?"active":""}" data-atf="all">All (${b})</button>
            <button class="pchip ${T.filter==="active"?"active":""}" data-atf="active">Pending (${I})</button>
            <button class="pchip ${T.filter==="completed"?"active":""}" data-atf="completed">✓ Completed (${w})</button>
          </div>
          ...
          <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap">
            <div style="display:flex;align-items:center;gap:6px">
              ...
              <select id="admTaskPersonFilter" style="font-size:12.5px;padding:6px 12px;border:1px solid var(--border-sm);border-radius:var(--r-md);background:var(--bg-surface);color:var(--text-1);outline:none">
                <option value="all" ${T.personnelId==="all"?"selected":""}>All Employees (${e.length})</option>
                ${e.map(r=>`
                  <option value="${r._id}" ${T.personnelId===r._id?"selected":""}>${escapeHtml(r.name)} (${r.role||r.department||"Staff"})</option>
                `).join("")}
              </select>
            </div>
            ...
            <div class="ticket-search-box" style="margin:0">
              ...
              <input type="text" id="admTaskSearchInp" placeholder="Search tasks or employee…" value="${escapeHtml(T.search)}" style="font-size:12px;padding:5px 8px">
              ${T.search?'<button type="button" id="admTaskClearSearch" style="background:none;border:none;color:var(--text-4);cursor:pointer;font-size:11px">✕</button>':""}
            </div>
      ...
      `:`
        <div style="display:flex;flex-direction:column;gap:18px">
          ${f.map(r=>{const $=r.tasks.length,E=r.tasks.filter(C=>C.status==="Completed").length,H=$>0?Math.round(E/$*100):0,B=$>0&&E===$;return`
              <div class="daily-checklist-card">
                ...
                <div style="padding:16px 20px;background:var(--bg-surface);border-bottom:1px solid var(--border-sm);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px">
                  ...
                  <div style="display:flex;align-items:center;gap:12px">
                    <span style="font-size:12.5px;font-weight:700;color:${B?"var(--green-600)":"var(--text-2)"}">
                      ${E} of ${$} completed (${H}%)
                    </span>
                    <div style="width:120px;height:8px;background:var(--bg-elevated);border-radius:var(--r-full);overflow:hidden">
                      <div style="width:${H}%;height:100%;background:${B?"var(--green-500)":"var(--brand-500)"};transition:width 0.4s ease"></div>
                    </div>
                    ${E>0?`
                ...
                <div>
                  ${r.tasks.map(C=>{const _=C.status==="Completed",D=C.dueDate?new Date(C.dueDate).toISOString().slice(0,10):"";return`
                      <div class="daily-item-row ${_?"completed":""}" style="padding:12px 20px">
                        <div style="display:flex;align-items:center;gap:14px;flex:1;min-width:0">
                          <button type="button" class="daily-circle-check ${_?"checked":""}" onclick="adminToggleTask('${C._id}')" title="${_?"Mark Incomplete":"Mark Complete"}">
                            ✓
                          </button>
                          <div style="flex:1">
  ...
  `;const y=document.getElementById("admDailyPrevDayBtn");y&&(y.onclick=()=>{T.date=oe(T.date,-1),x()});const a=document.getElementById("admDailyNextDayBtn");a&&(a.onclick=()=>{T.date=oe(T.date,1),x()});const m=document.getElementById("admDailyTodayBtn");m&&(m.onclick=()=>{T.date=n,x()});const A=document.getElementById("admDailyAllDatesBtn");A&&(A.onclick=()=>{T.date=T.date==="all"?n:"all",x()});const P=document.getElementById("admDailyDatePickerInp");P&&(P.onchange=r=>{r.target.value&&(T.date=r.target.value,x())}),document.querySelectorAll("[data-atf]").forEach(r=>{r.onclick=()=>{T.filter=r.dataset.atf,x()}});const N=document.getElementById("admTaskPersonFilter");N&&(N.onchange=r=>{T.personnelId=r.target.value,x()});const q=document.getElementById("admTaskSearchInp");q&&(q.oninput=r=>{T.search=r.target.value,pe(t)});const o=document.getElementById("admTaskClearSearch");o&&(o.onclick=()=>{T.search="",x()})}window.adminToggleTask=async function(t){try{await apiPatch("/tasks/"+t+"/toggle",{}),flashToast("Task status updated"),x()}catch(s){flashToast(s.message,!0)}};window.adminDeleteTask=async function(t){if(confirm("Delete this task?"))try{await apiDelete("/tasks/"+t),flashToast("Task deleted"),x()}catch(s){flashToast(s.message,!0)}};window.adminClearCompletedTasks=async function(t){if(confirm("Delete completed tasks?"))try{const s={date:T.date};t&&t!=="all"?s.personnelId=t:T.personnelId&&T.personnelId!=="all"&&(s.personnelId=T.personnelId);const e=await apiPost("/tasks/clear-completed",s);flashToast(`Completed tasks deleted (${e.deletedCount||0} removed) 🗑️`),x()}catch(s){flashToast(s.message,!0)}};me();
